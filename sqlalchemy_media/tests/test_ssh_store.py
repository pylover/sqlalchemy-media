...
class SSHStoreTestCase(MockupSSHTestCase):
    ...
    def test_put_from_stream(self):
        store = SSHStore(self.create_ssh_client(), self.base_url)
        target_filename = 'temp/test_put_from_stream/a/b/c/file_from_stream1.txt'
        content = b'Lorem ipsum dolor sit amet'
        stream = io.BytesIO(content)
        length = store.put(target_filename, stream)
        self.assertEqual(length, len(content))
        self.assertTrue(exists(join(self.here, target_filename)))
